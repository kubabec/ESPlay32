...
bool MessageUDP::isValid()
{
    ...
    return ((id != -1) && (!(ipAddress == (IPAddr){0,0,0,0})));
}
...
bool MessageUDP::toByteArray(byte* bufferPtr, size_t sizeCheck)
{
    ...
    if(sizeCheck == totalSize && bufferPtr != nullptr) /* check if received bufferPtr size is expected */
    {
        ...
        if(dataBuffer.size() > 0){ /* There are any data in the buffer */
            ...
            {
                bufferPtr[i]  = dataBuffer.at(i-10); /* copy all buffer data */
            }
        }
...
void MessageUDP::serialPrintMessageUDP(MessageUDP& msg)
{
    ...
    Serial.println("ID: " + String(msg.id));
    Serial.println("TotalSize : " + String(msg.totalSize));
    Serial.println("IP Address: " + String(msg.ipAddress.element1) + "." + String(String(msg.ipAddress.element2)) + "." + String(msg.ipAddress.element3) + "." + String(msg.ipAddress.element4));
